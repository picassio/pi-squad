...
- **Large-squad review reports**: when the finish notification says the complete report was written to `review-report.md`, that file — not the digest — is the review input. Read the ENTIRE file (Read tool with offset/limit until the end) before submitting any `squad_review` verdict; the digest's task index and pointer lines are navigation aids, not evidence.
- **Parallel writers → worktrees**: when multiple tasks edit the same repository concurrently, their descriptions should direct each agent into its own git worktree (`squad/<task-id>` branch), with one integration task merging and cleaning up. At review time, verify the repo is clean: `git worktree list` shows only the main tree and no stale `squad/*` branches remain — leftover worktrees accumulate gigabytes and have caused disk-full incidents.
