...
**Worktree isolation**: When other agents are concurrently writing to the same repository, each editing agent is instructed to work in its own git worktree on a `squad/<task-id>` branch (`git worktree add ../<repo>.wt-<task-id> -b squad/<task-id>`), hand off the branch name, and leave merging to the integration/final task — which merges all `squad/*` branches and cleans up every worktree (`git worktree remove` + branch delete). The planner adds these instructions to task descriptions whenever two or more tasks modify the same repo in parallel; read-only tasks stay in the main tree. Review-time check: `git worktree list` shows only the main tree.
